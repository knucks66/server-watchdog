...
eval "$(sed -n '/^select_stale_workers() {/,/^}/p' "$HERE/../scripts/load-guard.sh")"
...
' "$FIXTURE_OUTAGE" | select_stale_workers $THREE_H)"
...
check "aged past 3h: exactly the three workers"   "2084769:13380s
2084853:13370s
2084884:13370s" "$(printf '%s
' "$FIXTURE_AGED" | select_stale_workers $THREE_H)"
...
check "never selects Runner.Listener"   "" "$(echo '1319 901282 /opt/github-runner-rumio-2/bin/Runner.Listener run --startuptype service' | select_stale_workers $THREE_H)"

# A normal job must survive. The longest real job on this box is ~50 min.
check "healthy 50-minute worker survives"   "" "$(echo '999 3000 /opt/github-runner/bin.2.336.0/Runner.Worker spawnclient 1 2' | select_stale_workers $THREE_H)"

# Long-lived infrastructure must never be selected, however old.
check "postgres at 10 days is never selected"   "" "$(echo '8298 901272 postgres: checkpointer' | select_stale_workers $THREE_H)"
...
check "bare vite build is never selected"   "" "$(echo '2087476 99999 node /opt/github-runner/_work/rumio/rumio/node_modules/.bin/vite build' | select_stale_workers $THREE_H)"

# Boundary: strictly greater than, so exactly-at-threshold survives.
check "exactly at threshold survives"   "" "$(echo "42 $THREE_H /opt/github-runner/bin/Runner.Worker x" | select_stale_workers $THREE_H)"
check "one second past threshold is selected"   "42:10801s" "$(echo "42 $((THREE_H+1)) /opt/github-runner/bin/Runner.Worker x" | select_stale_workers $THREE_H)"

# Malformed ps lines must be skipped, not crash or match.
check "non-numeric etimes is skipped"   "" "$(echo 'abc xyz /opt/github-runner/bin/Runner.Worker x' | select_stale_workers $THREE_H)"
check "empty input yields nothing"   "" "$(printf '' | select_stale_workers $THREE_H)"
